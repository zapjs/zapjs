#!/usr/bin/env bun
/**
 * Comprehensive Fastify vs ZapJS Comparison
 *
...
const main = async () => {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('  Fastify vs ZapJS - Comprehensive Deep Dive');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('');
    ...
    // Test Fastify
    console.log('\x1b[36m━━━ Fastify ━━━\x1b[0m\n');
    let fastifyProc = await startServer('Fastify', 'node', ['servers/fastify-server.cjs'], 3002);
    ...
    for (const scenario of SCENARIOS) {
        ...
        for (const connections of CONNECTION_COUNTS) {
            ...
            // Warmup
            await runWrk(`http://localhost:3002${scenario.path}`, connections, WARMUP_DURATION, THREADS);

            // Actual test
            const output = await runWrk(`http://localhost:3002${scenario.path}`, connections, DURATION, THREADS);
    ...
    for (const scenario of SCENARIOS) {
        ...
        console.log('Connections │ Fastify RPS │ ZapJS RPS   │ Speedup │ Fastify Lat │ ZapJS Lat');
        console.log('─'.repeat(70));

        for (const connections of CONNECTION_COUNTS) {
            const fastifyResult = allResults.find(
                r => r.framework === 'Fastify' && r.scenario === scenario.name && r.connections === connections
            );
            const zapResult = allResults.find(
                r => r.framework === 'ZapJS' && r.scenario === scenario.name && r.connections === connections
            );

            if (fastifyResult && zapResult) {
                const speedup = zapResult.rps / fastifyResult.rps;
                const speedupColor = speedup >= 3 ? '\x1b[32m' : speedup >= 2 ? '\x1b[33m' : '\x1b[31m';

                console.log(
                    `${connections.toString().padStart(11)} │ ` +
                    `${(fastifyResult.rps / 1000).toFixed(1).padStart(9)}k │ ` +
                    `${(zapResult.rps / 1000).toFixed(1).padStart(9)}k │ ` +
                    `${speedupColor}${speedup.toFixed(2)}x\x1b[0m`.padEnd(20) + `│ ` +
                    `${fastifyResult.latency_avg_ms.toFixed(2).padStart(9)}ms │ ` +
    ...
    const fastifyResults = allResults.filter(r => r.framework === 'Fastify');
    const zapResults = allResults.filter(r => r.framework === 'ZapJS');

    const avgFastifyRPS = fastifyResults.reduce((sum, r) => sum + r.rps, 0) / fastifyResults.length;
    const avgZapRPS = zapResults.reduce((sum, r) => sum + r.rps, 0) / zapResults.length;
    const overallSpeedup = avgZapRPS / avgFastifyRPS;

    const minSpeedup = Math.min(
        ...SCENARIOS.flatMap(s =>
            CONNECTION_COUNTS.map(c => {
                const exp = allResults.find(r => r.framework === 'Fastify' && r.scenario === s.name && r.connections === c);
    ...
    console.log(`📈 Average Fastify RPS: ${(avgFastifyRPS / 1000).toFixed(1)}k`);
    ...
    const scenarioAverages = SCENARIOS.map(scenario => {
        ...
        const fastifyAvg = scenarioResults
            .filter(r => r.framework === 'Fastify')
    ...
    const reportPath = `../reports/fastify-deep-dive_${timestamp}.json`;
