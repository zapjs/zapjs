#!/usr/bin/env bun
/**
 * Comprehensive Express.js vs ZapJS Comparison
 *
...
const main = async () => {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('  Express.js vs ZapJS - Comprehensive Deep Dive');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('');
    ...
    // Test Express.js
    console.log('\x1b[33m━━━ Express.js ━━━\x1b[0m\n');
    let expressProc = await startServer('Express.js', 'node', ['servers/express-server.cjs'], 3001);
    ...
    for (const scenario of SCENARIOS) {
        ...
        for (const connections of CONNECTION_COUNTS) {
            ...
            // Warmup
            await runWrk(`http://localhost:3001${scenario.path}`, connections, WARMUP_DURATION, THREADS);

            // Actual test
            const output = await runWrk(`http://localhost:3001${scenario.path}`, connections, DURATION, THREADS);
    ...
    for (const scenario of SCENARIOS) {
        ...
        console.log('Connections │ Express RPS │ ZapJS RPS   │ Speedup │ Express Lat │ ZapJS Lat');
        console.log('─'.repeat(70));

        for (const connections of CONNECTION_COUNTS) {
            const expressResult = allResults.find(
                r => r.framework === 'Express.js' && r.scenario === scenario.name && r.connections === connections
            );
            const zapResult = allResults.find(
                r => r.framework === 'ZapJS' && r.scenario === scenario.name && r.connections === connections
            );

            if (expressResult && zapResult) {
                const speedup = zapResult.rps / expressResult.rps;
                const speedupColor = speedup >= 3 ? '\x1b[32m' : speedup >= 2 ? '\x1b[33m' : '\x1b[31m';

                console.log(
                    `${connections.toString().padStart(11)} │ ` +
                    `${(expressResult.rps / 1000).toFixed(1).padStart(9)}k │ ` +
                    `${(zapResult.rps / 1000).toFixed(1).padStart(9)}k │ ` +
                    `${speedupColor}${speedup.toFixed(2)}x\x1b[0m`.padEnd(20) + `│ ` +
                    `${expressResult.latency_avg_ms.toFixed(2).padStart(9)}ms │ ` +
    ...
    const expressResults = allResults.filter(r => r.framework === 'Express.js');
    const zapResults = allResults.filter(r => r.framework === 'ZapJS');

    const avgExpressRPS = expressResults.reduce((sum, r) => sum + r.rps, 0) / expressResults.length;
    const avgZapRPS = zapResults.reduce((sum, r) => sum + r.rps, 0) / zapResults.length;
    const overallSpeedup = avgZapRPS / avgExpressRPS;

    const minSpeedup = Math.min(
        ...SCENARIOS.flatMap(s =>
            CONNECTION_COUNTS.map(c => {
                const exp = allResults.find(r => r.framework === 'Express.js' && r.scenario === s.name && r.connections === c);
    ...
    console.log(`📈 Average Express.js RPS: ${(avgExpressRPS / 1000).toFixed(1)}k`);
    ...
    const scenarioAverages = SCENARIOS.map(scenario => {
        ...
        const expressAvg = scenarioResults
            .filter(r => r.framework === 'Express.js')
    ...
    const reportPath = `../reports/express-deep-dive_${timestamp}.json`;
